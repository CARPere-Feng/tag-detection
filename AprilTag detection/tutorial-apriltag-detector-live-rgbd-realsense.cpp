...
//! [Macro defined]
  ...
  try {
    ...
    g.acquire(reinterpret_cast<unsigned char *>(I_color.bitmap), reinterpret_cast<unsigned char *>(I_depth_raw.bitmap),
              NULL, NULL, &align_to_color);//error, must comment out lines related to "config.disable_stream"
    ...
    std::cout << cam << std::endl; //cam.get_u0()
    ...
    for (;;) {
      ...
      std::vector<vpHomogeneousMatrix> cMo_vec; // Detection using color only
      ...
      for (size_t i = 0; i < tags_corners.size(); i++) {
        vpHomogeneousMatrix cMo; // Detection using color + depth
        ...
        if (vpPose::computePlanarObjectPoseFromRGBD(depthMap, tags_corners[i], cam, tags_points3d[i], cMo, &confidence_index)) {
          ...
          vpDisplay::displayText(I_color2, 35 + i*100, 20, ss.str(), vpColor::red);
          ss2 << "Pose = " << cMo[0][0] << " " << cMo[0][1] << " " << cMo[0][2] << " " << cMo[0][3];
          vpDisplay::displayText(I_color2, 55 + i*100, 20, ss2.str(), vpColor::red);
          ss3 << "       " << cMo[1][0] << " " << cMo[1][1] << " " << cMo[1][2] << " " << cMo[1][3];
          vpDisplay::displayText(I_color2, 70 + i*100, 20, ss3.str(), vpColor::red);
          ss4 << "       " << cMo[2][0] << " " << cMo[2][1] << " " << cMo[2][2] << " " << cMo[2][3];
          vpDisplay::displayText(I_color2, 85 + i*100, 20, ss4.str(), vpColor::red);
          ss5 << "       " << cMo[3][0] << "    " << cMo[3][1] << "    " << cMo[3][2] << "    " << cMo[3][3];
          vpDisplay::displayText(I_color2, 100 + i*100, 20, ss5.str(), vpColor::red);
        }
      }
      //! [Pose from depth map]
